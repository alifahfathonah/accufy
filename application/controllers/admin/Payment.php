...
class Payment extends Home_Controller {
    ...
    public function convert_currency($amount='', $from_currency='', $invoice_id)
    {
        ...
        $rate = (1 / get_rate($from_currency)) * get_rate($invoice->currency_code);

        if ($from_currency == $invoice->currency_code) {
            $conversion = '';
            $convert_total = str_replace(",", "", $result);
        } else {
            $conversion = trans('currency-conversion').': ' . $invoice->currency_symbol . ' ' . number_format($result, 2) . ' (' . $invoice->currency_code . ') at ' . number_format($rate, 4);
            $convert_total = str_replace(",", "", $result);
        }
        return number_format($convert_total, 2);
    }
    ...
    public function stripe_payment($invoice_id, $amount, $cus_amount='')
    {
        ...
        //$amount = round($invoice->grand_total); 
        $amount = round($amount); 
